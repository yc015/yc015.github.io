...
$(document).ready(async () =>   {
    ...
    tsParticles.load("landing", myJson).then((container) => {
        container.canvas.element.addEventListener("click", (e) => {
            ...
            particles.forEach(particle => {
                ...
                // particle.size.value = particle.options.size.value;
            });

            if (closestParticle) {
                console.log(closestParticle.shapeData.src)
                if (closestParticle.shapeData.src.includes("harvard")) {
                    window.open("https://seas.harvard.edu/person/yida-chen")
                }
                else if (closestParticle.shapeData.src.includes("sun")) {
                    window.open("https://www.pixiv.net/users/9695349")
                }
                else if (closestParticle.shapeData.src.includes("lulu")) {
                    window.open("https://github.com/yc015/TalkTuner-a-dashboard-for-chatbot-llm")
                }
            }
        });
      });
    ...
    tsParticles.setOnClickHandler((event, particles) => {
        // Example: Log particle details or do something with the particle information
        console.log("Particle clicked", particles);
    });
